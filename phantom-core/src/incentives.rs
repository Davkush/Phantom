...
/// Task 3.3: Reciprocal Routing Incentives (Phase 1)
///
...
pub struct RoutingIncentives {
    ...
    node_stats: RwLock<HashMap<Vec<u8>, RoutingStats>>,
    /// Configuration
...
impl RoutingIncentives {
    ...
    pub fn new(own_public_key: HybridPublicKey) -> Self {
        let key_bytes = serialize_public_key(&own_public_key);
        Self {
            node_stats: RwLock::new(HashMap::new()),
            config: IncentiveConfig::default(),
            own_public_key: key_bytes,
        }
    }

    /// Create with custom configuration.
    pub fn with_config(own_public_key: HybridPublicKey, config: IncentiveConfig) -> Self {
        let key_bytes = serialize_public_key(&own_public_key);
        Self {
            node_stats: RwLock::new(HashMap::new()),
            config,
    ...
    pub async fn record_outbound(&self, node_key: &HybridPublicKey, bytes: u64) {
        let mut stats = self.node_stats.write().await;
        let node_stats = stats.entry(serialize_public_key(node_key)).or_default();
        node_stats.record_outbound(bytes);
    }

    /// Record inbound traffic for a specific node.
    pub async fn record_inbound(&self, node_key: &HybridPublicKey, bytes: u64) {
        let mut stats = self.node_stats.write().await;
        let node_stats = stats.entry(serialize_public_key(node_key)).or_default();
    ...
    pub async fn check_routing_permission(&self, node_key: &HybridPublicKey) -> Result<(), RoutingDenied> {
        let key_bytes = serialize_public_key(node_key);
        let stats = self.node_stats.read().await;
        let node_stats = stats.get(&key_bytes).ok_or_else(|| RoutingDenied::NoHistory)?;
    ...
    pub async fn get_routing_ratio(&self, node_key: &HybridPublicKey) -> f64 {
        let key_bytes = serialize_public_key(node_key);
        let stats = self.node_stats.read().await;
        match stats.get(&key_bytes) {
    ...
    pub async fn get_stats(&self, node_key: &HybridPublicKey) -> Option<RoutingStats> {
        let key_bytes = serialize_public_key(node_key);
        let stats = self.node_stats.read().await;
        stats.get(&key_bytes).cloned()
    }
...
/// Helper to serialize HybridPublicKey to bytes for use as HashMap key
fn serialize_public_key(key: &HybridPublicKey) -> Vec<u8> {
    let mut bytes = Vec::with_capacity(32 + key.kyber_pub.len());
    bytes.extend_from_slice(&key.x25519_pub);
    bytes.extend_from_slice(&key.kyber_pub);
    bytes
}
...
pub struct IncentiveAwareRouter {
    ...
    fallback_nodes: RwLock<Vec<HybridPublicKey>>,
...
impl IncentiveAwareRouter {
    ...
    pub fn new(own_key: HybridPublicKey) -> Self {
        Self {
    ...
    pub async fn add_fallback_node(&self, node_key: HybridPublicKey) {
    ...
    pub async fn select_route(
        &self,
        available_nodes: &[HybridPublicKey],
        path_length: usize,
    ) -> Result<Vec<HybridPublicKey>, RouteSelectionError> {
        let mut rng = rand::thread_rng();

        // Filter nodes that have routing permission
        let mut viable: Vec<HybridPublicKey> = Vec::new();
...
mod tests {
    ...
    async fn test_routing_permission_new_node() {
        let key = crate::hybrid_kem::HybridKeyPair::generate().public_key();
