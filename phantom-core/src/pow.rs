...
/// Proof of Work solution containing the nonce and derived salt
#[derive(Debug, Clone)]
...
/// Derives a deterministic salt from the challenge using blake3
/// This ensures the same challenge always produces the same salt for verification
fn derive_salt(challenge: &[u8]) -> [u8; 16] {
    let hash = blake3::hash(challenge);
    let mut salt = [0u8; 16];
    salt.copy_from_slice(&hash.as_bytes()[0..16]);
    salt
}
...
/// Returns the nonce and salt that solves the puzzle, or None if computational limit reached.
pub fn solve_static_pow(challenge: &[u8], difficulty: u32) -> Option<PowSolution> {
    ...
    // Derive deterministic salt from challenge to ensure reproducibility
    let salt = derive_salt(challenge);
    ...
    for i in 0..10_000u64 {
        ...
        let password_hash = argon2.hash_password(&input, &salt_string).unwrap();
        let hash_bytes = password_hash.hash.unwrap();

        if check_difficulty(hash_bytes.as_bytes(), difficulty) {
            return Some(PowSolution { nonce, salt });
        }
    }
    None
}
...
mod tests {
    ...
    fn test_dht_admission_stress() {
        ...
        let is_valid = verify_static_pow(challenge, &solution.nonce, &solution.salt, difficulty);
        assert!(is_valid, "DHT Node PoW Admission must be mathematically valid");

        // 3. Reject tampering
        let mut tampered_nonce = solution.nonce;
        tampered_nonce[0] ^= 0xFF;
        let is_invalid = verify_static_pow(challenge, &tampered_nonce, &solution.salt, difficulty);
